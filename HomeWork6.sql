...
DELIMITER //
CREATE PROCEDURE task1(id_archive INT)
BEGIN
	INSERT INTO users_old (id, firstname, lastname, email)
    SELECT u.id, u.firstname, u.lastname, u.email
    FROM users u
    WHERE u.id = id_archive;
    DELETE 
    FROM users u
    WHERE u.id = id_archive;
COMMIT;
...
SELECT 
	IF(CURRENT_TIME >= '06:00:00' AND  CURRENT_TIME < '12:00:00', 'Good morning',
		IF(CURRENT_TIME >= '12:00:00' AND  CURRENT_TIME < '18:00:00', 'Good day',
			IF(CURRENT_TIME >= '18:00:00' AND  CURRENT_TIME < '00:00:00', 'Good evening', 'Good night')
            )
		)
END INTO task2;
RETURN task2;
...
/*
ENGINE=ARCHIVE
it has some serious limitations. Apart from lack of support for DELETE, REPLACE and UPDATE 
statements (which may be acceptable for some needs), another one is that it does not allow you 
to have indexes, although you can have an auto_increment column being either a unique or non-unique index.
*/

/*
TRIGGER
https://dev.mysql.com/doc/refman/8.0/en/trigger-syntax.html
USE trigger with a table, to activate for INSERT operations
*/

CREATE TABLE logs
(
	created_date DATETIME DEFAULT now(),
    in_table VARCHAR(25) NOT NULL,
    id_PK BIGINT NOT NULL
)
ENGINE=ARCHIVE;

CREATE TRIGGER users_t
AFTER INSERT ON users 
FOR EACH ROW
INSERT INTO logs SET in_table = 'users', id_PK = NEW.id;

CREATE TRIGGER communities_t
AFTER INSERT ON communities
FOR EACH ROW
INSERT INTO logs SET in_table = 'communities', id_PK = NEW.id;

CREATE TRIGGER messages_t
AFTER INSERT ON messages
FOR EACH ROW
INSERT INTO logs SET in_table = 'messages', id_PK = NEW.id;
...
VALUES 
('Alex', 'Dark', '562653@example.org'),
('Alice', 'Friday', 'toi@example.org');
...
INSERT INTO messages  (from_user_id, to_user_id, body, created_at) 
VALUES
(11, 12, 'Hello. Check trigger.',  DATE_ADD(NOW(), INTERVAL 1 MINUTE)),
(12, 11, 'Hello. Check Archive Engine.',  DATE_ADD(NOW(), INTERVAL 3 MINUTE));
